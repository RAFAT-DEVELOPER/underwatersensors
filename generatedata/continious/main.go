...
func ContinuousDataGeneration(sensors []models.Sensor) {
	...
	for _, sensor := range sensors {
		_, epipelagicOK := generatedata.EpipelagicSensorGroupNames[strings.Split(sensor.Codename, " ")[0]]
		if epipelagicOK {
			epipelagicSensors = append(epipelagicSensors, sensor)
			//fmt.Println("found value:", sensor.Codename)
		}
		_, mesopelagicOK := generatedata.MesopelagicSensorGroupNames[strings.Split(sensor.Codename, " ")[0]]
		if mesopelagicOK {
			mesopelagicSensors = append(mesopelagicSensors, sensor)
			//fmt.Println("found value:", sensor.Codename)
		}
		_, bathypelagicOK := generatedata.BathypelagicSensorGroupNames[strings.Split(sensor.Codename, " ")[0]]
		if bathypelagicOK {
			bathypelagicSensors = append(bathypelagicSensors, sensor)
			//fmt.Println("found value:", sensor.Codename)
		}
		_, abyssopelagicOK := generatedata.AbyssopelagicSensorGroupNames[strings.Split(sensor.Codename, " ")[0]]
		if abyssopelagicOK {
			abyssopelagicSensors = append(abyssopelagicSensors, sensor)
			//fmt.Println("found value:", sensor.Codename)
		}
		_, trenchesOK := generatedata.AbyssopelagicSensorGroupNames[strings.Split(sensor.Codename, " ")[0]]
		if trenchesOK {
	...
	go GenerateObservations(generatedata.EpipelagicDataRate, epipelagicSensors, observationsCh)
	go GenerateObservations(generatedata.MesopelagicDataRate, mesopelagicSensors, observationsCh)
	go GenerateObservations(generatedata.BathypelagicDataRate, bathypelagicSensors, observationsCh)
	go GenerateObservations(generatedata.AbyssopelagicDataRate, abyssopelagicSensors, observationsCh)
	go GenerateObservations(generatedata.TrenchesDataRate, trenchesSensors, observationsCh)
...
func GenerateObservationsData(datarate int, sensors []models.Sensor) (observations []models.CreateSensorObservationRequest) {
	...
	switch datarate {
	case generatedata.EpipelagicDataRate:
		for _, sensor := range sensors {
			...
			fishSpeciesObserved := generatedata.GenerateRandomInt(0, len(epipelagicSpecies)/10+2)
			...
			for _, element := range randomIndexes {
				observation := models.CreateSensorObservationRequest{
					...
					Count:              generatedata.GenerateRandomInt(1, 6),
					DetectionTimeFrom:  observationTime,
					DetectionTimeUntil: observationTime.Add(time.Duration(generatedata.EpipelagicDataRate) * time.Second),
				}
				observations = append(observations, observation)
			}
		}
	case generatedata.MesopelagicDataRate:
		for _, sensor := range sensors {
			...
			fishSpeciesObserved := generatedata.GenerateRandomInt(0, len(mesopelagicSpecies)/10+2)
			...
			for _, element := range randomIndexes {
				observation := models.CreateSensorObservationRequest{
					...
					Count:              generatedata.GenerateRandomInt(1, 6),
					DetectionTimeFrom:  observationTime,
					DetectionTimeUntil: observationTime.Add(time.Duration(generatedata.MesopelagicDataRate) * time.Second),
				}
				observations = append(observations, observation)
			}
		}
	case generatedata.BathypelagicDataRate:
		for _, sensor := range sensors {
			...
			fishSpeciesObserved := generatedata.GenerateRandomInt(0, len(bathypelagicSpecies)/10+2)
			...
			for _, element := range randomIndexes {
				observation := models.CreateSensorObservationRequest{
					...
					Count:              generatedata.GenerateRandomInt(1, 6),
					DetectionTimeFrom:  observationTime,
					DetectionTimeUntil: observationTime.Add(time.Duration(generatedata.BathypelagicDataRate) * time.Second),
				}
				observations = append(observations, observation)
			}
		}
	case generatedata.AbyssopelagicDataRate:
		for _, sensor := range sensors {
			...
			fishSpeciesObserved := generatedata.GenerateRandomInt(0, len(abyssopelagicSpecies)/10+2)
			...
			for _, element := range randomIndexes {
				observation := models.CreateSensorObservationRequest{
					...
					Count:              generatedata.GenerateRandomInt(1, 6),
					DetectionTimeFrom:  observationTime,
					DetectionTimeUntil: observationTime.Add(time.Duration(generatedata.AbyssopelagicDataRate) * time.Second),
				}
				observations = append(observations, observation)
			}
		}
	case generatedata.TrenchesDataRate:
		for _, sensor := range sensors {
			...
			fishSpeciesObserved := generatedata.GenerateRandomInt(0, len(trenchesSpecies)/10+2)
			...
			for _, element := range randomIndexes {
				observation := models.CreateSensorObservationRequest{
					...
					Count:              generatedata.GenerateRandomInt(1, 6),
					DetectionTimeFrom:  observationTime,
					DetectionTimeUntil: observationTime.Add(time.Duration(generatedata.TrenchesDataRate) * time.Second),
				}
				observations = append(observations, observation)
			}
		}
...
func calculateEpipelagicTransparency(depth float64) int {
	...
	return int(int64(transparency + generatedata.GenerateRandomFloat(-3.0, 3.0)))
}
...
func calculateMesopelagicZoneTransparency(depth float64) int {
	...
	return int(transparency + generatedata.GenerateRandomFloat(-2.0, 2.0))
}
...
func calculateBathypelagicZoneTransparency(depth float64) int {
	...
	return int(transparency + generatedata.GenerateRandomFloat(-1.0, 1.0))
}
...
func calculateTrenchesZoneTransparency(depth float64) int {
	...
	return int(transparency + generatedata.GenerateRandomFloat(-0.5, 0.5))
}
